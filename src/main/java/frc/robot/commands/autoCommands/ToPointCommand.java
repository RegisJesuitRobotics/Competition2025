...
private final TunableTelemetryProfiledPIDController translationController = new TunableTelemetryProfiledPIDController("/drive/auto", Constants.AutoConstants.pointTranslationGains, Constants.AutoConstants.trapPointTranslationGains);
private final Pose2dEntry desiredPoseEntry = new Pose2dEntry("/drive/neuralDesiredPose", true);
private final SimpleMotorFeedforward ffController = Constants.AutoConstants.pointTranslationFFGains.createFeedforward();
 ...
 public ToPointCommand(CommandSwerveDrivetrain drive, Supplier<Pose2d> desiredPoseSupplier)
    ...
    public void execute()
    {
       ...
       if (getTranslationError().getNorm() > .05){
          double translationFeedback = translationController.calculate(-getTranslationError().getNorm());
          double translationFF = translationController.getSetpoint().velocity;

          translationVeloctiy = new Translation2d(translationFeedback + translationFF, getTranslationError().getAngle());
       }

       Translation2d finalTranslationVeloctiy = translationVeloctiy;
       drive.applyRequest(() -> swerveRequest.withVelocityX(finalTranslationVeloctiy.getX()).withVelocityY(finalTranslationVeloctiy.getY()).withRotationalRate(0.0));
